...
std::vector<std::pair<int, const char*>> guns =
{
	...
	{ weapon_sg553,			"#SFUI_WPNHUD_SG556" },
...
void SkinLocalizer::localizeGuns()
{
	int idx = -1;
	...
	for (auto it : guns)
	{
		...
		std::string localized = converter.to_bytes(p_Localize->findSafe(it.second));
		guns[idx] = { it.first, strdup(localized.c_str()) };
	}
}

void SkinLocalizer::localizeKnives()
{
	int idx = -1;
	...
	for (auto it : knives)
	{
		...
		std::string localized = converter.to_bytes(p_Localize->findSafe(it.second));
		knives[idx] = { it.first, strdup(localized.c_str()) };
	}
}

void SkinLocalizer::localize()
{
